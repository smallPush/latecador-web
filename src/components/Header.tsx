...
export const Header: React.FC = () => {
  ...
  const menuItems = [
    { label: 'Qui som', href: '#about', isExternal: false },
    { label: 'El nostre Menú', href: '#menu', isExternal: false },
    { label: 'Les nostres especialitats', href: '/especialitats', isExternal: true },
    { label: 'Contacte', href: '#contact', isExternal: false },
  ];

  const handleNavClick = (e: React.MouseEvent<HTMLAnchorElement>, item: { label: string, href: string, isExternal: boolean }) => {
    if (item.isExternal) {
      // For the separate page, let Link or navigate handle it
      return;
    }
    ...
    if (location.pathname !== '/') {
      ...
      // If we are on home page, scroll normally
  ...
  const isEspecialitatsPage = location.pathname === '/especialitats';

  return (
    <>
      {/* Navigation Bar */}
      <nav
        className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-500 px-6 py-4 ${isScrolled || isEspecialitatsPage ? 'bg-brand-900/90 backdrop-blur-xl shadow-lg' : 'bg-transparent'
          }`}
      >
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <Link to="/" className="font-serif text-2xl text-white group">
            ...
            {menuItems.map((item) => (
              item.isExternal ? (
                <Link
                  key={item.label}
                  to={item.href}
                  className={`text-white hover:text-gold-400 text-xs font-black uppercase tracking-[0.2em] transition-colors ${location.pathname === item.href ? 'text-gold-400' : ''}`}
                >
                  {item.label}
                </Link>
              ) : (
        ...
        {menuItems.map((item) => (
          item.isExternal ? (
            <Link
              ...
              to={item.href}
              onClick={() => setIsMenuOpen(false)}
              className={`text-white hover:text-gold-400 text-4xl font-serif italic transition-colors ${location.pathname === item.href ? 'text-gold-400' : ''}`}
            >
              {item.label}
            </Link>
          ) : (
